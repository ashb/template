...
var Test = function Template$Test() {
}

exports.Test = Test;

Test.prototype.__build_case = function(opt) {
  return function() {
    //this.expect(2);
    ...
    this.same(output, expect);
  }
};

  // This is the main testing sub-routine.
Test.prototype.build_tests = function (data, tproc, params) {
  var tprocs = [];
  ...
  for (var i in tests) {
    ...
    input = input.replace(
      ...
      function(whole,n) {
        ...
        if (tproc_) {
          ...
          system.stderr.print("no such template object to use:", n);
        return '';
      }
    );
    ...
    this['test_TT_Input_' + (name || count)] = this.__build_case(test);
  }
}


Test.prototype.callsign = function() {
  var c = {};
